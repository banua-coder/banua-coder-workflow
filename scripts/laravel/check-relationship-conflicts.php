 ...
 *
 * Usage: php scripts/check-relationship-conflicts.php [models-dir]
 */
...
if (! empty($conflicts)) {
    ...
    foreach ($conflicts as $conflict) {
        echo "  [{$conflict['type']}] {$conflict['model']}\n";
        echo "    File: {$conflict['file']}\n";
        echo "    {$conflict['message']}\n";
        echo "    Fix: Rename the relationship method to avoid shadowing the column.\n";
...
if (! empty($warnings)) {
    ...
    foreach ($warnings as $warning) {
        echo "  [{$warning['type']}] {$warning['model']}\n";
...
if ($hasErrors) {
    echo "Please fix the conflicts above to prevent runtime bugs.\n\n";
    exit(1);
}
