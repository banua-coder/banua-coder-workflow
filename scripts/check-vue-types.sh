...
done < <(find resources/js -name "*.vue" -type f)

if [ $found_issues -eq 0 ]; then
    echo -e "${GREEN}✓ No interface/type definitions found in Vue files.${NC}"
    exit 0
else
    echo -e "${RED}✗ Found issues in $found_issues file(s).${NC}"
    echo ""
    echo "Please move these definitions to resources/js/types/ directory."
